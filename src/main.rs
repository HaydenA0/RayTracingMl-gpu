...
fn main() {
    ...
    let camera = camera::Camera::new(width as f32 / height as f32, 10.0);
    let spheres_data = vec![spheres::Sphere::new([0.0, 0.0, -1.0], 0.5)];

    let shader_source = gpu::load_shaders("src/shaders/", vec!["*"]);
